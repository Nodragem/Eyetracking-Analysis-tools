...
E_SACC_TYPE = np.array((1, 1, 2)) ## Expected Saccade in each condition (trial type)
...
class TrialPlotter:
    ...
    def drawEyeMovement(self, df, page_number):
        ...
        for key, grp in df.ix[select, :].groupby(TRIAL_COL_NAME):  # not really pretty to not check the size
            ...
            trial_type = grp["trial type"].iloc[0]
...
def isSuspect(trial, rawdata=[], progress_bar = (0, None) ):
    ...
    type_trial = trial.ix[0, "trial_type"]
    print trial_ID
...
def placeMarkers(condition, position_mat, threshold_amplitude): ## to be continued
    ...
    startings = np.where(d == -1)[0][:]+1
    #startings = np.array((np.arange(len(startings))*2+1, startings)).T
    #print startings
...
def computeMarkers(grp, time_only = False, progress_bar = (0, None)):
    ...
    list_markers = placeMarkers(threshold_acc & threshold_velocity & (grp["stimuliON"]>0), grp[["xp", "yp"]], threshold_amplitude)
    ...
    if time_only:
        ...
        for i, line in list_markers.iterrows():
            ...
            if time_event == -1:
                ...
                try:
                    newline = grp.ix[grp["time"] == time_event, [TRIAL_COL_NAME,"trial type","target dir","time", "xp", "yp"]].values.tolist()[0]
                    #print newline + [marker]
                    mat.append(newline + [marker])
                except Exception as e:
                    print "There may be duplicate trials on this DataFrame"
                    print e
                    print grp.ix[grp["time"] == time_event, [TRIAL_COL_NAME,"trial type","target dir","time", "xp", "yp"]].values
                    exit()
        
        return pd.DataFrame(mat, columns = ["trial_ID","trial_type","target_dir","time", "xp", "yp", "marker"])
