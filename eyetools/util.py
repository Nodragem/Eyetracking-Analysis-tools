...
def selectByHalfDistance(df, halfdist, section = "both", stimulus = "S1"): ## return a boolean mask
    if halfdist == None:
        return True
    abs_dir = abs(df[stimulus+"_DIR"])
    print abs_dir
...
def checkRejected(seqdata, trajdata, name="NO NAME"):
    if "rejected" in seqdata.columns:
        ...
        trajdata = trajdata.ix[trajdata["TRIAL_ID"].isin(good_trials), :]
...
def selectIfInRectangle(traj_grouped, center, radius, point_to_test="end"):
    ...
    print "position of point", point_to_test, ": (", p[0], ";", p[1], ")",
    #return pd.DataFrame(columns=traj_grouped.columns)
    # print traj_grouped.columns
    if len(center) < 2:
...
def extractOneSaccadeRT(grp_seqdata, saccade = 2, reference = None, col_name = "RT", return_value_only = False):
    if reference is None:
        marker_ref = (saccade - 1)*2 ## this marker is the end of the previous saccade
    else:
        marker_ref = reference

    marker_saccade = (saccade - 1)*2 + 1
    ## here, the second selection, we take only trial with a second saccade end
    trial_nb = grp_seqdata["TRIAL_ID"].head(1).values[0]
    #print "\r ", trial_nb,
    if np.sum(grp_seqdata.marker == marker_saccade+1) != 1: ## the end of the saccade doesn't not exist
    ...
    ref_zero = grp_seqdata.ix[grp_seqdata.marker == marker_ref, "time"].values[0] ## end of the previous saccade
    saccade_start = grp_seqdata.ix[grp_seqdata.marker == marker_saccade, "time"].values[0] ## saccade_start of the wanted saccade
    RT = saccade_start - ref_zero
    df[col_name] = RT
    #print df
    if return_value_only:
        return df[col_name]
...
def extractOneSaccadeTraj(grp_seqdata, trajdata, saccade = 2, section="saccade"):
    ...
    if section == "saccade":
        selection = (trajdata["TRIAL_ID"] == trial_nb) & (trajdata["time"] < end) & (trajdata["time"] > start)
    elif section == "fixation":
        selection = (trajdata["TRIAL_ID"] == trial_nb) & (trajdata["time"] < start) & (trajdata["time"] > zero)
    elif section == "all":
        selection = (trajdata["TRIAL_ID"] == trial_nb) & (trajdata["time"] < end) & (trajdata["time"] > zero)
    #print trajdata.columns
    return trajdata.ix[selection,:]

def select_slice(tab, time1, time2, f = lambda x:x):
    t2 = time2.ix[time2["TRIAL_ID"].values == tab["TRIAL_ID"].head(1).values, "time"].values[0]
    t1 = time1.ix[time1["TRIAL_ID"].values == tab["TRIAL_ID"].head(1).values, "time"].values[0]
...
def computeAngleDeviation(tab, bins):
    ...
    return pd.DataFrame({"dev": np.mean(f(new_radius)),"std": np.std(f(new_radius).std())}, index=[tab["TRIAL_ID"].head(1).values])
